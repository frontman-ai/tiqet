The general project documentation is in: /home/bluehotdog/GoogleDrive/dannifriedland/knowledge/Projects/Tiqet
make sure you explore and read it
## The Essence Of Style

> “There are three things extremely hard: steel, a diamond, and to know one's self.” — Benjamin
> Franklin

Tiqet's coding style is evolving. A collective give-and-take at the intersection of
...
In other words, Tiqet has a “zero technical debt” policy. We do it right the first time. This
...
Tiqet is a core library with thin executable wrappers. `libs/core` owns the product behavior: state
configuration, storage layout, Git state/worktree operations, task persistence, sync semantics, and
all future domain rules. `apps/cli` must stay a wrapper around that core. The CLI may parse argv,
format stdout/stderr, choose exit codes, and wire process/IO dependencies, but it must not contain
Tiqet business logic or duplicate state contracts.

When adding behavior, implement and test it in `libs/core` first, then expose it through the CLI.
Public interfaces such as `initialize()` may be tested through `apps/cli/src/e2e.zig` when the CLI
is only exercising the core behavior and not duplicating it.
A reference to core that only keeps the dependency alive is not enough; the installed executable
must call core for real work.
...
- **Assertions detect programmer errors. Unlike operating errors, which are expected and which must
  ...
  - **[Pair assertions](https://Tiqet.com/blog/2023-12-27-it-takes-two-to-contract).** For
  ...
  - **Assert the relationships of compile-time constants** as a sanity check, and also to document
    and enforce [subtle
    invariants](https://github.com/coilhq/Tiqet/blob/db789acfb93584e5cb9f331f9d6092ef90b53ea6/src/vsr/journal.zig#L45-L47)
    or [type
    sizes](https://github.com/coilhq/Tiqet/blob/578ac603326e1d3d33532701cb9285d5d2532fe7/src/ewah.zig#L41-L53).
...
- Reserve core working-memory backing storage statically at startup. **Command-scoped allocations
  must use that reserved storage, with no runtime heap growth or fallback.** A
  `std.heap.FixedBufferAllocator` can allocate tables and retained strings within the fixed budget.
  Exhaustion must return an error. Discard all command-owned references before resetting the
  allocator on success or failure. Keep per-record parser scratch separate and copy retained data
  into command storage. Fixed backing storage bounds memory use, but does not prevent dangling
  slices. Tests must cover ownership, exhaustion, and reuse across commands.
...
- Be explicit. Minimize dependence on the compiler to do the right thing for you.
  ...
  `self` (see [an example](https://github.com/Tiqet/Tiqet/blob/0.16.19/src/lsm/compaction.zig#L1932-L1937)).
...
- Don't overload names with multiple meanings that are context-dependent. For example, Tiqet
...
- Be on your guard for **[buffer bleeds](https://en.wikipedia.org/wiki/Heartbleed)**. This is a
  ...
  deterministic guarantees as required by Tiqet to be violated.
...
Tiqet has **a “zero dependencies” policy**, apart from the Zig toolchain. Dependencies, in
...
At the end of the day, keep trying things out, have fun, and remember—it's called Tiqet, not
